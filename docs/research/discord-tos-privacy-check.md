...
| フェーズ      | 実装前提                                                                                                               |
| ------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Dogfood Alpha | 自前Discord、疑似ログ、サンプルログで分類・週報・通知・FAQ候補・限定自動返信を検証し、外部導入と一般Hosted公開はしない |
| Closed Beta   | Phase 1で実装済みの機能を信頼できる少数協力者に限定して評価し、導入告知・保存期間・削除依頼導線を個別確認する          |
...
- 保存期間。
- 削除リクエストの連絡先または手順。
- 自動返信の有無、AIキャラクターによる補助回答であること、公式回答ではないこと、回答許可範囲。
- 自動BAN、ユーザー評価を行わないこと。
- 管理者向けに整理・共有する用途であること。

## Dogfood Alpha 必須要件

- 読み取り専用モードを初期状態にしつつ、限定自動返信のON/OFF、許可範囲、エスカレーション条件を実装する。
- 対象チャンネルを明示的に選択させる。
...
- 保存期間を設定できるようにする。
- LLMに送るデータは必要最小限にする。
- 自動返信は管理者が明示的にONにしたチャンネルとカテゴリだけで実行する。
- 法務、広報、料金、障害、ロードマップ、個別アカウント、課金、セキュリティ、ルール処分は自動回答せず運営者へエスカレーションする。
- 自動返信本文、参照元、判定理由、送信状態、管理者フィードバックを確認できるようにする。
