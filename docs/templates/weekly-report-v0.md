...
- 自動返信: {{auto_reply_sent_count}}件 / エスカレーション: {{auto_reply_escalated_count}}件
...
| 種別 | 件数 | 内容 | 確認事項 |
| --- | --- | --- | --- |
| {{auto_reply_type}} | {{count}} | {{summary}} | {{review_note}} |
...
- 自動返信は送信件数、承認待ち件数、エスカレーション件数、誤返信・不要返信フィードバックを分けて書く。
...
- DMは読まない。
- ユーザー評価や自動処分はしない。
- AIキャラクターの補助回答と公式回答を分離し、公式判断が必要なものは人間の運営者が判断する。
