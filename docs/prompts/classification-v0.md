...
日本語Discord投稿を、運営者が確認しやすい形式へ分類する。このプロンプトは Dogfood
Alpha の分類検証用であり、API契約ではない。
...
- 自動BANや処分判断はしない。
- この分類タスクでは返信文を生成しない。Phase 1 の自動返信は `docs/prompts/auto-reply-v0.md` の別タスクで、ON/OFF、許可範囲、エスカレーション条件を通して制御する。
- 投稿本文にない事実を補わない。
- JSONだけを返す。
```
...
外部導入やHosted公開へ進む前の正式合格ラインは `proposal.md` 10.4 を使う。
